...
"""Code-injection PoC for eval/exec sinks."""
...
class CodeInjectionExecTemplate(BaseTemplate):
    ...
    def preconditions(self, candidate: VulnerabilityCandidate, repo_model: RepoModel) -> bool:
        if not super().preconditions(candidate, repo_model):
            return False
        return candidate.sink_metadata.is_code_exec is True

    def _payload(self, candidate: VulnerabilityCandidate, sentinel: str, is_exploit: bool) -> str:
        # Per spec §6.7: the payload is a Python expression, not a shell command.
        if is_exploit:
